...
void CubeObject::DrawCube(Matrix mvp) {

	//glUseProgram(m_objectCubeShader->program);
...
void CubeObject::SetUpCubeMesh(Mesh mesh) {
	m_objectMesh = mesh;
}

void CubeObject::SetUpCubeTexture(CubeTexture* texture) {
	m_objectCubeText = new CubeTexture[m_CubeObjectTextCount];
	for (int i = 0; i < m_CubeObjectTextCount; i++)
	{
		m_objectCubeText[i] = texture[m_CubeObjectTextID[i]];
	}
}

void CubeObject::SetUpCubeShader(CubeShaders* shader) {
	m_objectCubeShader = &shader[m_CubeObjectShaderID];
}
